...
class StreamlitSmokeTests(unittest.TestCase):
    def test_matched_form_uses_offline_fallback(self):
        ...
        self.assertTrue(any('AI сейчас не используется' in item.value for item in app.caption))
        self.assertTrue(any(item.label == 'Подходит' and item.value == '5' for item in app.metric))
        self.assertTrue(any(item.label == 'Показано' and item.value == '3' for item in app.metric))
        self.assertTrue(any(item.label == 'Режим' and item.value == 'Local' for item in app.metric))
        self.assertTrue(any('стабильном порядке' in item.value for item in app.caption))
        self.assertTrue(any(item.label == 'Как работает AI-агент' for item in app.expander))
    ...
    def test_ai_mode_separates_python_facts_from_verified_evidence(self):
        ...
        with patch('agent.run_matching', return_value=result):
            ...
            app.button[0].click().run()
        self.assertFalse(app.exception)
        self.assertTrue(any('✨ AI Agent active' in item.value for item in app.markdown))
        self.assertTrue(any(item.label == 'Режим' and item.value == 'AI Agent' for item in app.metric))
        self.assertTrue(any(item.label == 'Как работает AI-агент' for item in app.expander))
        ...
        for card in result['cards']:
            self.assertEqual(1, sum(card['selected_evidence'] in item.value for item in app.info))
    ...
    def test_rare_case_shows_actual_count_and_optional_language(self):
        ...
        app.button[0].click().run()
        self.assertFalse(app.exception)
        self.assertEqual(1, sum(card['anon_name'] == item.value for card in app.session_state['last_result']['cards'] for item in app.subheader))
    ...
    def test_plan_b_scenario_f_keeps_original_query_and_shows_both_options(self):
        ...
        self.assertTrue(any('ДРУГАЯ ДАТА' in item.value for item in app.markdown))
